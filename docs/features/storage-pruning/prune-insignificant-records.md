...
- Storage-size monitoring / alerts when approaching the 10 MB quota (the bar is informational, not an alert).
- Undo / restore after deletion.
- Cross-group "select all" / "deselect all" (per-group select is implemented; global is not).
