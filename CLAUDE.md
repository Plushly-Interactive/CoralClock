...
- `chrome.storage.local` preference keys go in `src/shared/prefKeys.js`, and read-query dispatch tags (the page-side provider `type` selectors) go in `src/shared/queryTypes.js`, *only when used by more than one file*. A key used in a single file stays as a local string literal in that file — no constant, no shared-module entry. Promote to a shared constant the moment a second file needs it. Same rule for any future "named string" of this kind.
